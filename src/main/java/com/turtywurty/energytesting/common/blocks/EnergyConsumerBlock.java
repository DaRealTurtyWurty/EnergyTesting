...
public class EnergyConsumerBlock extends Block {

	public static final DirectionProperty HORIZONTAL_FACING = BlockStateProperties.HORIZONTAL_FACING;

	public EnergyConsumerBlock(Properties builder) {
		super(builder);
	...
	public TileEntity createTileEntity(BlockState state, IBlockReader world) {
		return TileEntityTypeInit.CONSUMER.get().create();
	}
	...
	public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player,
		...
		if (!worldIn.isRemote) {
			if (worldIn.getTileEntity(pos) instanceof ConsumerTileEntity) {
				ConsumerTileEntity tile = (ConsumerTileEntity) worldIn.getTileEntity(pos);
				ServerPlayerEntity serverPlayer = (ServerPlayerEntity) player;
				IContainerProvider provider = ConsumerContainer.getServerContainerProvider((ConsumerTileEntity) tile,
						pos);
				INamedContainerProvider namedProvider = new SimpleNamedContainerProvider(provider,
						((ConsumerTileEntity) tile).getDisplayName());
